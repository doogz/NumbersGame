...
// It defines an interface for playing a gamePlayer, INumbersGamePlayer, and provides one standard implementation, NumbersGamePlayer.
// The reference algorithms uses the SDK's NumbersGamePlayer; faster implementation are possible.
namespace ScottLogic.NumbersGame.Game
{
    /// <summary>
    /// SDK implementation of INumbersGamePlayer.
    ...
    /// </summary>
    public class NumbersGamePlayer : INumbersGamePlayer
    {
        // The current state of the gamePlayer is simply modelled as a list of integers
        ...
        public NumbersGamePlayer()
        {
            // Note that the private fields (above) are initialised beforehand
        }

        public NumbersGamePlayer(IEnumerable<int> initVals, int target = 0)
            :this()
        {
        ...
        public NumbersGamePlayer(NumbersGamePlayer gamePlayer)
        {
            _numbers.AddRange(gamePlayer._numbers);
            _history.AddRange(gamePlayer._history);
            Target = gamePlayer.Target;
        }
        ...
        // ---------------------------
        // INumbersGamePlayer implementation
        // ----------------------------
        ...
        public INumbersGamePlayer CreateDescendent(IOperation op)
        {
            var game = new NumbersGamePlayer(this); // Deep copy
            game.DoOperation(op);
            return game;
        }

        public List<INumbersGamePlayer> CreateAllDescendents()
        {
            var games = new List<INumbersGamePlayer>();
